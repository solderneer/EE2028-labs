...
int main(void) {
	...
	send_uartstring(LPC_UART3, "Start\n");

	while(1) {
		switch(mode) {
			case INITIALIZATION_MODE:
				oled_putString(0, 0, "INITIALIZATION...", OLED_COLOR_WHITE, OLED_COLOR_BLACK);
				oled_putString(0, 10, "Press TOGGLE", OLED_COLOR_WHITE, OLED_COLOR_BLACK);
				oled_putString(0, 20, "to climb ", OLED_COLOR_WHITE, OLED_COLOR_BLACK);
				if(sw3Flag) {
			...
			case CLIMB_MODE:
				...
				while(1) {
					...
					if(light < LIGHT_THRESHOLD) {
						...
						flag |= 0x02;
						bar_strength = ((LIGHT_THRESHOLD - light) / 60); // Scaling to 16 bits
